...
function init_nm_woo_gateway(){
	...
	class WC_Gateway_NM_TwoCheckout extends WC_Payment_Gateway {
		...
		function get_price($price){
			...
			if($this -> xchange_rate == 'yes' ){
				
				$all_rates = $this -> get_rates_from_yahoo();
				
				if( $all_rates ) {
					
					$local_currency	= get_woocommerce_currency();
					if ( $all_rates[$local_currency] ) {
						$local_currency_rate = $all_rates[$local_currency];
						$price = $price / $local_currency_rate;
					}
